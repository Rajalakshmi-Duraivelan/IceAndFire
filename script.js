...
inputField.setAttribute('id','search-box');
inputField.setAttribute('placeholder','Enter your search');
inputField.setAttribute('className','form-control form-control-lg');
inputField.setAttribute("list", "ddl");

// To display Suggestions while typing in search ox using datalist
const ddl = document.createElement("datalist");
ddl.setAttribute('id','ddl');

// Append input in search box container
formGroup.appendChild(inputField);
formGroup.appendChild(ddl);

// On First Load of the script file
(async () => {
  const res = await fetch("https://anapioficeandfire.com/api/books");
  const books = await res.json();
  const suggestion = document.getElementById("ddl");
  books.map((book) => {
  ...
  try {
    ...
    if (filteredBooks.length > 0) {
      filteredBooks.map(async (book) => {
        ...
        value.setAttribute("style","margin-left: 34px;")
        ...
        value.setAttribute("style","margin-left: 25px;")
        ...
        value.setAttribute("style","margin-left: 55px;")
        ...
        value.setAttribute("style","margin-left: 45px;")
        ...
        value.setAttribute("style","margin-left: 48px;")
...
button.setAttribute("style","background-color:red; border:2px solid black");
